...
let resultP1 = calculateTax(10000, 3000);
...
function sendNotification(email) {
  ...
  let name = email.slice(0, ind);
  return `${name} sent you an email from ${email.slice(ind + 1)}`;
}
let resultP2=sendNotification("zihad@gmail.com");
...
function checkDigitsInName(name) {
  ...
  return all.includes("number") ? true : false;
}
let resultP3=checkDigitsInName("n9ayeem")
...
function calculateFinalScore(obj) {
  if (typeof obj !== "object" || Array.isArray(obj)) {
    ...
    let nm = typeof obj.name;
    let ts = typeof obj.testScore;
    let sg = typeof obj.schoolGrade;
    let isf = typeof obj.isFFamily;
    ...
    if (
      ...
      let familyPoin = obj.isFFamily === true ? 20 : 0;
      let sum = obj.testScore + obj.schoolGrade + familyPoin;

      return sum >= 80 ? true : false;
    } else {
      return "Invalid Input";
    }
  }
}
let resultP4=calculateFinalScore({ name: "Rajib", testScore: 45,  schoolGrade: 25, isFFamily : false })
...
function waitingTime(waitingTimes, serialNumber) {
  if (Array.isArray(waitingTimes) && !isNaN(serialNumber)) {
    ...
    if (waitingTimes.length < serialNumber) {
      for (let v of waitingTimes) {
        sum += v;
      }
      let avrg = Math.round(sum / waitingTimes.length);

      let wait = serialNumber - 1 - waitingTimes.length;
      let nowWait = avrg * wait;
      return nowWait;
    } else {
      return "Invalit Input";
    }
...
let resultP5=waitingTime([13, 2, 6, 7, 10], 6)
console.log(resultP5)
